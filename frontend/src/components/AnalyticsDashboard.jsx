...
const AnalyticsDashboard = () => {
  ...
  if (loading || !data) {
    return <div className="p-8 text-center text-muted-foreground animate-pulse">{t('loadingStats')}</div>;
  }

  const statusData = [
    { name: t('toDoStatus'), value: data.todo },
    { name: t('inProgressStatus'), value: data.in_progress },
    { name: t('completedStatus'), value: data.completed },
  ].filter(d => d.value > 0);
    ...
    <div className="flex flex-col gap-8">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-border bg-card p-6 text-center">
          <p className="text-sm font-medium text-muted-foreground uppercase">{t('totalTasks')}</p>
        ...
        <div className="rounded-2xl border border-border bg-card p-6 text-center">
          <p className="text-sm font-medium text-muted-foreground uppercase">{t('completionRate')}</p>
          <p className="text-4xl font-bold text-emerald-500 mt-2">
            {data.total > 0 ? Math.round((data.completed / data.total) * 100) : 0}%
          </p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-6 text-center">
          <p className="text-sm font-medium text-muted-foreground uppercase">{t('workingStats')}</p>
          <p className="text-4xl font-bold text-blue-500 mt-2">{data.in_progress}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-6 text-center">
          <p className="text-sm font-medium text-muted-foreground uppercase">{t('categories')}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="rounded-2xl border border-border bg-secondary/20 p-6">
          <h3 className="mb-6 font-semibold">{t('statusDistribution')}</h3>
          <div className="h-[300px] w-full">
            ...
            ) : <p className="text-center mt-10 text-muted-foreground">{t('noEnoughData')}</p>}
          </div>
        </div>

        <div className="rounded-2xl border border-border bg-secondary/20 p-6">
          <h3 className="mb-6 font-semibold">{t('tasksByCategory')}</h3>
          <div className="h-[300px] w-full">
            {catData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
            ...
            ) : <p className="text-center mt-10 text-muted-foreground">{t('createCategoriesToView')}</p>}
          </div>
        </div>
      </div>
    </div>
